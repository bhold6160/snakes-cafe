...
class Order:
    ...
    def __str__(self):
        ...
        """
        sub_total = self.calculate_total()
        order_tax_total = self.calculate_tax()
    ...
    def __len__(self):
        return sum([count for count in self.basket.values()])
...
***********************************""")
    ...
    def check_user_input(self, menu):
        """
        ...
        if user_input == 'q':
            ...
            order.remove_item(removed_item)
        elif user_input == 'menu':
            self.print_menu()
        elif user_input.title() in menu:
            self.item_category(user_input)
        # elif user_input.title() not in menu:
        #     check_valid_input(user_input)
        else:
            user_input = user_input.split(' ')
            if len(user_input) == 1:
                order.add_order(user_input)
            else:
                order.add_order(user_input[0], user_input[1])
        return user_input
    ...
    def item_category(self, category):
        """
...
#     return order_summary
    ...
    def calculate_total(self):
        """
        Calculating the total number of items multiplied by their quantity
        """
        order_total = 0
        for item, quantity in basket.items():
            item = item.title()
    ...
    def calculate_tax(self):
        ...
        """
        tax_total = self.calculate_total() * tax
        return tax_total
    ...
    def main(self):
        ...
        while True:
            user_input = self.check_user_input(menu)
